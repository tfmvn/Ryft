...
class PMApp:
    ...
    def _splash(self) -> None:
        ...
        console.print()
        wm = Text()
        wm.append("  pm", style=f"bold {VIOLET}")
...
def render_doctor(checks: list[tuple[str, bool, str]]) -> None:
    ...
    for label, ok, detail in checks:
        row = Text()
        row.append(f"  {_I['check'] if ok else _I['cross']}  ",
                   style=MINT if ok else CORAL)
        row.append(f"{label:<22}", style=TEXT_HI if ok else TEXT_DIM)
        row.append(detail, style=TEXT_DIM)
        console.print(row)
    _sp()
