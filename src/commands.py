...
# # # Decrypted from pm/formatter.py:
# # In cmd_format()
...
def cmd_doctor(ctx, args: list[str]) -> None:
    ...
    checks = [
        ...
        ("Ollama Node",     ctx.ai.is_available(),           cfg.ollama.host),
        ("Commit Model",    True,                            cfg.ollama.commit_model),
        ("Analysis Model",  True,                            cfg.ollama.analysis_model),
        ("Configuration",   cfg.path is not None,            str(cfg.path) if cfg.path else "Using defaults"),
        ("Sync Watcher",    bool(ctx.sync and ctx.sync.is_running), "Active" if ctx.sync and ctx.sync.is_running else "Stopped"),
    ]
...
COMMANDS: dict[str, CommandSpec] = {
    ...
    "doctor":   CommandSpec("doctor",   cmd_doctor,   "Run health checks.",              usage=["/doctor"]),
    ...
    "exit":     CommandSpec("exit",     cmd_exit,     "Exit PM.",                        usage=["/exit"]),
}
